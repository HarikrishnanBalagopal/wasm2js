...
export const setupModAotDemo = async (compiledJSCode: string) => {
  ...
  const AUDIO_FRAMES: number = 4096;
  const fontSize: number = 16;
  const numIns: number = 31;
  const numRows: number = 64;
  const numInsCols: number = 2;

  let dv: DataView | undefined = undefined;
  ...
  //   let number = window.location.search.slice(1) | 0;
  ...
  for (let i = 0; i < songs.length; ++i) {
    ...
    optionEl.textContent = songs[i].slice(hash + 1);
    optionEl.value = `${i}`;
  ...
  volumeEl.addEventListener('input', (event: any) => {
    gainNode.gain.setValueAtTime(event.target.value / 100, audio.currentTime);
  });

  monoEl.addEventListener('change', (event: any) => {
    gainNode.disconnect();
    audio.destination.channelCount = event.target.checked ? 1 : 2;
    gainNode.connect(audio.destination);
  });

  function init(channels: number) {
    const decoder = new TextDecoder();
    function getString(offset: number, len: number) {
    ...
    rowEls = [];
    {
      ...
      for (let row = 0; row < numRows; ++row) {
        ...
        const channelEls: Array<RowCellData> = [];
        for (let ch = 0; ch < numChannels; ++ch) {
          const tdEl = document.createElement('td');
  ...
  function draw(p: number, r: number) {
    newPattern(p);
    newRow(r);
  }
  ...
  const hex = (x: number): string => x.toString(16).toUpperCase();
  const hex2 = (x: number): string => hex(x).padStart(2, '0');
  function newPattern(p: number) {
